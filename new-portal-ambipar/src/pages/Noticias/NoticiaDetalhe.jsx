...
export default function NoticiaDetalhe() {
    ...
    // helper simples para montar URL completa
    const getImgUrl = (img) => `${STRAPI_URL}${img}`;
    ...
    return (
        <section className="max-w-4xl mx-auto px-6 py-16">
            ...
            {info.imagem_capa?.url && (
                <img
                    src={getImgUrl(info.imagem_capa.formats?.large?.url || info.imagem_capa.url)}
            ...
            {relacionadas.length > 0 && (
                <div className="mt-16">
                    <h2 className="text-2xl font-semibold text-[#B2CC21] dark:text-[#CDFF00] mb-6">
                        Veja também:
                    </h2>
                        ...
                        {relacionadas.map((item) => (
                            ...
                            >
                                {item.imagem_capa?.url && (
                                    <img
                                        src={getImgUrl(item.imagem_capa.formats?.small?.url || item.imagem_capa.url)}
